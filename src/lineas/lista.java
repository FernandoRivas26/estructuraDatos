...
public class lista {//CLASE PUBLICA LISTA
	...
	public boolean delete(int pos){//METODO PARA ELIMINAR UN NODO
		...
		if(isEmpty()){//SI LA LISTA ESTA VACIA AUTOMATICAMENTE ENCIAREMOS UN FALSE YA QUE NO HAY VALORES POR ELIMINAR
			...
			nodo temporal=this.encontrar(pos-1);//CREAREMOS UNA VARIABLE DE TIPO NODO LLAMADA TEMPORAL CON UN CALOR INICIAL DE EL NODO QUE SEGRESE EL METODO ENCONTRAR 
	...
	public void insertarPos(int valor, int pos){//METODO PARA INSERTAR UN NODO EN UNA POSICION ESPECIFICA
		if(pos>0 && this.contar()>0 && pos<=this.contar()){
			...
			}else if(pos==contar()){//DEº LOCONTRARIO SI POS ES IGUAL A EL NUMERO DE NODOS, ES DECIR A LA ULTIMA POSICION
				this.insertarFinal(valor);//MANDAMOS LLAMAR AL METOOD INSERTARFINAL CON EL VALOR A INSERTAR
			}else{//DE LO CONTRARIO
			nodo anterior = this.encontrar(pos-1);//CREAREMOS UNA VARIABLE DE TIPO NODO CON UN VALOR INICIAL A LO QUE RETORNE EL METODO ENCONTRAR EN DICHA POSOCION
	...
	public nodo encontrar(int pos){//METODO QUE REGRESA EL VALOR DE UN NODO EN UNA POSICION ESPECIFICA
		nodo temporal=null, anterior=this.cabeza;//CREAMOS DOS VARIABLES DE TIPO NODO UNA LLAMADA TEMPORAL Y OTRA ANTE
		int i=0;//INICIAMOS CON UN CONTADOR QUE INICIARA EN CERO, UTILIZADO PARA SABER EN QUE POSICION VAMOS 
		while(i<=pos){//MIENTRAS I SEA MENOR IGUAL A LA POSICION QUE BUSCAMOS
			i++;//INCREMENTO EN I
			anterior=temporal;//ANTERIOR ES IGUAL A EL NODOTEMPORAL QUE INICIO EN LA CABEZA
			temporal=temporal.objeterSig();//TEMPORAL SERA IAGUAL AL NODO QUE ESTA ENLAZADO A EL
		}
		return anterior;//SE REGRESA EL VALOR DEL NODO EN LA POSICION BUSCADO
	}
